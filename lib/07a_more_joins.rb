...
def heart_tracks
  ...
  execute(<<-SQL)
    SELECT
      albums.title, COUNT(tracks.song) AS heart_songs
    FROM
      albums
      JOIN tracks
        ON tracks.album = albums.asin
...
def rock_superstars
  ...
  execute(<<-SQL)
  SELECT
    albums.artist, COUNT(DISTINCT rock_albums.asin)
  FROM
    albums
    JOIN styles
      ON styles.album = albums.asin
    JOIN albums rock_albums
      ON rock_albums.asin = styles.album
  WHERE
    styles.style LIKE '%Rock%' AND
    rock_albums.artist = albums.artist
  GROUP BY
    albums.artist
  ORDER BY
    COUNT(rock_albums.*) DESC
  LIMIT 1
  SQL
end
...
def expensive_tastes
  ...
  FROM
    ...
    JOIN (
      SELECT
        albums.asin, albums.price, COUNT(tracks.*) as total_tracks
      FROM
        albums
        JOIN tracks
          ON tracks.album = albums.asin
      ...
      GROUP BY
        albums.asin
    ) AS track_count ON track_count.asin = album_styles.album
  GROUP BY
    album_styles.style
  ORDER BY
    (SUM(track_count.price) / SUM(track_count.total_tracks)) DESC
  LIMIT 5
  SQL
end
